...
class MarkupTests(unittest.TestCase):
    ...
    def test_real_version_and_document_counts(self):
        ...
        source = packages.ROOT / 'metadataExample/龍門石窟北魏紀年題記_詮釋資料＿正式-re3.xlsx'
        with tempfile.TemporaryDirectory() as tmp:
            packages.DATA = Path(tmp) / 'data'
            try:
                meta = packages.import_file(source)
                base = packages.DATA / meta['id']
                self.assertEqual(meta['count'], 153)
                facets = packages.read(base / 'facets.json')
                expected = {'PersonName': (455,126), 'LocName': (15,20), 'Office': (35,24),
                            'Udef_Buddhist_Term': (93,129), 'Udef_Cultural_Term': (13,7)}
                for name, (terms, docs) in expected.items():
                    values = facets['tag:'+name]
                    self.assertEqual(len(values), terms)
                    self.assertEqual(len(set(n for ids in values.values() for n in ids)), docs)
