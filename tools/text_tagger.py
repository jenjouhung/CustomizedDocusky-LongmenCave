...
# ---------------------------------------------------------------------------
# 使用者設定區：直接執行本程式時，修改這裡即可。
# 路徑可使用相對於本專案根目錄的相對路徑，或完整路徑。
# TARGET_SHEET = None 代表使用目標 Excel 的第一個工作表。
# ---------------------------------------------------------------------------
PROJECT_ROOT = Path(__file__).resolve().parent.parent
TARGET_FILE = PROJECT_ROOT / "metadataExample" / "龍門石窟北魏紀年題記_詮釋資料＿正式-re2.xlsx"
TARGET_SHEET: str | None = None
TARGET_COLUMN = "題記錄文"
TAG_LIST_FILE = PROJECT_ROOT / "metadataExample" / "龍門北魏題記_各類詞彙總表.xlsx"
OUTPUT_FILE = PROJECT_ROOT / "metadataExample" / "龍門石窟北魏紀年題記_詮釋資料＿正式-re3.xlsx"
...
def parse_args() -> argparse.Namespace:
    ...
    parser.add_argument("--target-file", type=Path, default=TARGET_FILE)
    parser.add_argument("--target-sheet", default=TARGET_SHEET)
    parser.add_argument("--target-column", default=TARGET_COLUMN)
    parser.add_argument("--tag-list-file", type=Path, default=TAG_LIST_FILE)
